//provide token to access Mapbox API with
mapboxgl.accessToken = 'pk.eyJ1IjoiYW5hbWFyaWlheiIsImEiOiJjbGRtMTR5YmUwMjBqM3VrZDU0N2RmeTVuIn0.TtYMegWHD_9XSk_jO1jZFg'; //Add default public map token from your Mapbox account 
...
const map = new mapboxgl.Map({
    container: 'map1', //ID for div where map is embedded in HTML file
...
map.on('load', () => {
    ...
    //add a geojson file source "bikways" for Toronto bikeways
...
//note that this variable was created (despite having the data source linked in the above map) in order to allow parsing through the file and assign custom markers to the points' coordinates
...
for (const feature of universities.features) {
    ...
    //create a marker for each feature (with style indicated by the "marker" class and 
    //location indicated by each feature's point coordinates) and add to the "map" variable
...
const map2 = new mapboxgl.Map({
    container: 'map2', //ID for div where map is embedded in HTML file
...
for (const feature of universities.features) {
    ...
    //create a marker for each feature (with style indicated by the "marker" class and 
    //location indicated by each feature's point coordinates) and add to the "map2" variable
...
const map3 = new mapboxgl.Map({
    container: 'map3', //ID for div where map is embedded in HTML file
...
for (const feature of universities.features) {
    ...
    //create a marker for each feature (with style indicated by the "marker" class and 
    //location indicated by each feature's point coordinates) and add to the "map3" variable
